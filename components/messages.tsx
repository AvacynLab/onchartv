      ...
      <Conversation className="mx-auto flex min-w-0 max-w-4xl flex-col gap-4 md:gap-6">
        <ConversationContent className="flex flex-col gap-4 px-2 py-4 md:gap-6 md:px-4">
          ...
          {safeMessages.map((message, index) => {
             ...
             */
            /**
             * Track unique finance payload fingerprints to collapse duplicates emitted
             * while the assistant streams tool responses. The map remembers the index
             * of the retained part so we can replace transient placeholders with the
             * final snapshot once it arrives.
             */
            const financePartFingerprints = new Set<string>();
            const financePartIndexByFingerprint = new Map<string, number>();

            const computeFinanceFingerprint = (
              value: unknown,
              fallbackType: string
            ) => {
              const candidateType =
                ...
                typeof (value as { type?: unknown }).type === "string"
                  ? ((value as { type: string }).type as string)
                  : fallbackType;

              return `${candidateType}:${stableStringifyForHash(value)}`;
            };

            const isTransientFinancePart = (candidate: unknown) => {
              if (!candidate || typeof candidate !== "object") {
                return false;
              }

              const transientValue = (candidate as { transient?: unknown }).transient;
            ...
            const deduplicatedParts = Array.isArray(message.parts)
              ? message.parts.reduce<ChatMessage["parts"]>((acc, part) => {
                  ...
                  const canonicalFingerprint = computeFinanceFingerprint(
                    payload,
                    partType
                  );
                  const existingIndex = financePartIndexByFingerprint.get(
                    canonicalFingerprint
                  );

                  if (typeof existingIndex === "number") {
                    const existingPart = acc[existingIndex];
                    if (
                      isTransientFinancePart(existingPart) &&
                      !isTransientFinancePart(part)
                    ) {
                  ...
                  financePartIndexByFingerprint.set(
                    canonicalFingerprint,
                    acc.length
                  );
                  financePartFingerprints.add(canonicalFingerprint);
                  acc.push(part);
                  return acc;
                }, [])
              : [];
