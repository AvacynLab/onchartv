...
- 2025-12-20 — Normalised inline edit payloads that still surfaced legacy string fragments by updating the message editor helper and extending its Vitest coverage, ensuring Playwright receives the refreshed prompt content before regenerations.
- 2025-12-21 — Restored the missing `});` in the message editor spec and re-ran `pnpm exec vitest run tests/unit/components/message-editor.spec.tsx --coverage=false`, confirming all five tests pass with the new legacy-string coverage.
