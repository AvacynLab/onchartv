...
- **2025-10-18** — Gestion des requêtes d’édition sans métadonnée `clientTextSignature` : la route `/api/chat` compare désormais les signatures texte et privilégie les fragments entrants lorsqu’ils diffèrent de la base, tout en journalisant le cas legacy. Nouveau test dédié (`pnpm dlx vitest@2.1.9 run tests/unit/routes/chat.post.hermetic.spec.ts --no-coverage --reporter=basic`). Tentative Playwright ciblée (`pnpm exec playwright test tests/e2e/accessibility.spec.ts tests/e2e/chat.test.ts tests/e2e/reasoning.test.ts`) interrompue faute de dépendances front (`@tanstack/react-query`).
- **2025-10-18** — Défense contre les signatures client obsolètes : la route `/api/chat` vérifie désormais que le texte entrant correspond bien à la signature persistée avant de réutiliser l’ancien prompt. En cas de divergence nous journalisons l’anomalie et utilisons les fragments entrants, ce qui déverrouille les scénarios Playwright d’édition. Tests ciblés : `pnpm exec vitest run tests/unit/routes/chat.post.hermetic.spec.ts --no-coverage --reporter=basic`.
- **2025-10-18** — Synchronisation complète du flux d'édition : `MessageEditor` reconstruit désormais la signature client avant de relancer `regenerate` et fournit le message mis à jour via `prepareSendMessagesRequest`, garantissant que l'API reçoit toujours le prompt édité. Tests ciblés : `pnpm exec vitest run tests/unit/components/message-editor.spec.tsx tests/unit/components/chat.spec.tsx --no-coverage --reporter=basic`.
