...
describe("ai provider configuration", () => {
  ...
  it("streams a finance summary after chart tool completions", async () => {
    process.env.PLAYWRIGHT = "true";
    process.env.NEXT_PHASE = "phase-production-build";
    ...
    const prompt: ModelMessage[] = [
      {
        ...
        content: [
          {
            /**
             * Mirror the tool payload emitted by the Playwright finance mocks so we
             * confirm the inline provider produces the human-readable summary that
             * the UI renders as an assistant bubble.
             */
            type: "tool-result",
            toolCallId: "call_finance_chart",
            toolName: "tool.finance.chart.fetch",
